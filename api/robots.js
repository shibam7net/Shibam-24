...
function normalizeRobotsText(source) {
  ...
  const lines = raw
    ...
    .filter((line) => !/^disallow\s*:\s*\/dashboard\b/i.test(line));
...
function buildDefaultRobots() {
  return normalizeRobotsText([
    'User-agent: *',
    'Allow: /',
    'Disallow: /admin',
  ].join('\n'));
...
export default async function handler(_req, res) {
  ...
  try {
    ...
    const configured = seoSetting?.value?.robots_txt || buildDefaultRobots();
    return respond(res, 200, normalizeRobotsText(configured));
  } catch (error) {
    console.error('robots handler error', error);
    return respond(res, 200, buildDefaultRobots());
  }
}
